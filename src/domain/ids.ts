...
function randomUuid(): string {
  ...
  // Browsers expose randomUUID only in secure contexts (https or localhost).
  // getRandomValues has no such limit, so build an RFC 4122 version 4 id from it
  // when the app is served over plain http, for example to a phone on the LAN.
...
export function mintPlacementId(): PlacementId {
  return placementId(`placement-${randomUuid()}`)
}
